...
const selectedCountryCode = ref(user.country);

watch(() => selectedCountryCode.value, (newCountryCode) => {
    const dialCode = getDialingCode(newCountryCode);
    if (!dialCode) return;

    const allDialCodes = countries.map(c => getDialingCode(c.code));
    let phone = form.phone;
    allDialCodes.forEach(code => {
        phone = phone.replace(new RegExp('^\\+?' + code), '');
    });
    form.phone = `+${dialCode}${phone}`;
    form.country = newCountryCode; // set country for backend
});

function getDialingCode(countryCode) {
    const match = countries.find(c => c.code === countryCode);
    const codeMatch = match?.name.match(/\(\+(\d+)\)/);
    return codeMatch ? codeMatch[1] : '';
}

const validationErrors = ref({name: '', email: '', country: '', phone: '', passport_number: ''});
...
watch(() => form.email, debounce(async (email) => {
    ...
    if (!email) return validationErrors.value.email = 'Email is required.';
    if (!/^\S+@\S+\.\S+$/.test(email)) return validationErrors.value.email = 'Invalid email format.';
    if (email !== user.email) {
        ...
        <form @submit.prevent="submit" class="mt-6 space-y-6">
            <div>
                ...
                <TextInput id="name" v-model="form.name" class="mt-1 block w-full"
                           :class="{ 'border-red-500': validationErrors.name || form.errors.name }" autofocus/>
            ...
            <div>
                ...
                <select id="country" v-model="selectedCountryCode"
            ...
            <div>
                ...
                <TextInput id="passport_number" v-model="form.passport_number" class="mt-1 block w-full"
